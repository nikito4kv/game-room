...
export async function POST(request: Request) {
  ...
  // allowExpiredToken: токен здесь — лишь доказательство личности, а не грант на
  // вход. Хост остаётся в живой комнате дольше 30-минутного TTL токена (LiveKit
  // держит соединение), поэтому срок токена игнорируем — иначе модерация ломалась
  // бы через полчаса. Защита сохраняется: ник из токена всё равно должен совпасть
  // с актуальным hostIdentity (после передачи прав старый токен бесполезен).
